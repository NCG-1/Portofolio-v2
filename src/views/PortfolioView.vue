<template>
  <div class="px-5 py-5 md:px-12 md:py-10 text-left text-blue-50 mx-3">
    <article data-page="about">
      <header>
        <!-- Section for Past Project Experience in HTML -->
        <div class="text-2xl font-bold text-white mb-10 fadein-bot title-section flex items-center justify-center flex-col">
          <h4>Past Project Experience in HTML</h4>
          <h4 class="text-base font-normal text-transparent bg-clip-text bg-gradient-to-r from-slate-100 to-blue-300">
            Explore the projects I've worked on in HTML so far
          </h4>
        </div>
      </header>
      <section>
        <div class="grid grid-cols-1 gap-4 pb-32 md:grid-cols-2 xl:grid-cols-2 fade-zoom-in">
          <div v-for="item in itemsHTML" :key="item.id">
            <div
      ...
      <header>
        <!-- Section for Past Project Experience in Java -->
        <div class="text-2xl font-bold text-white mb-10 fadein-bot title-section flex items-center justify-center flex-col">
          <h4>Past Project Experience in Java</h4>
          <h4 class="text-base font-normal text-transparent bg-clip-text bg-gradient-to-r from-slate-100 to-blue-300">
            Explore the projects I've worked on in Java so far
          </h4>
        </div>
      </header>
      <section>
        <div class="grid grid-cols-1 gap-4 pb-32 md:grid-cols-2 xl:grid-cols-2 fade-zoom-in">
          <div v-for="item in itemsJava" :key="item.id">
            <div
              class="item-card flex flex-col items-center gap-2 rounded bg-[#1e1e1f] hover:bg-[#282828] border border-[#383838] rounded-xl text-blue-50 md:gap-3 px-5 py-5 lg:px-5 ">
              <div class="flex h-12 w-12 items-center justify-center p-0 h-full w-full lg:p-0 zoom-in">
                <img alt="Java" loading="lazy" decoding="async" data-nimg="1" class="drop-shadow-xl rounded rounded-xl"
                  :src="'/img/portfolio-' + item.imageUrl + '.png'">
              </div>
              <div class="w-full flex flex-col gap-2 items-center text-sm md:text-base lg:text-lg">
                <div class="title-text font-medium text-secondary">{{ item.name }}</div>
